...
def build_predictor_for_kernel(kernel_type, backend, init_sample_num = 1000, finegrained_sample_num = 10,
    ...
    # Cleanup per iteration to avoid keeping large per-sample ONNX artifacts.
    if os.path.isdir(kernels_dir):
        import shutil
        shutil.rmtree(kernels_dir, ignore_errors=True)
    ...
    for i in range(1, iteration):
        ...
        # Cleanup per iteration (after profiling+fit) to free disk space early.
        if os.path.isdir(kernels_dir):
            import shutil
            shutil.rmtree(kernels_dir, ignore_errors=True)
            os.makedirs(kernels_dir, exist_ok=True)
